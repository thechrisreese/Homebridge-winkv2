...
var model = {
	...
	cameras: require('./lib/model/camera')
};
				...
				return {
					get: function(callback) {
						GET({
							...
							} else if (device_group === "cameras" ) {
								model.cameras(data, wink);
					...
					update: function(data, callback) {
							...
							if (device_group == "securitysystem"){
								var desiredStatePath = "/cameras" + "/" + device_id + "/desired_state"
								console.log("Desired State Path: " + desiredStatePath)
							} else	{
								// try to output to the console so that we can see what's going on
								var desiredStatePath = "/" + device_group + "/" + device_id + "/desired_state"
								console.log("Desired State Path: " + desiredStatePath)
							}
						PUT({
							path: desiredStatePath,//"/" + device_group + "/" + device_id + "/desired_state",
